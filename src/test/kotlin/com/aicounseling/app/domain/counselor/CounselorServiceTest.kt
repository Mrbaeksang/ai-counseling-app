...
class CounselorServiceTest {
    ...
    fun findById_success() {
        ...
        every { ratingRepository.findByCounselorId(1L) } returns listOf(
            CounselorRating(
                id = 1L,
                user = testUser,
    ...
    fun findAllWithSort_rating() {
        ...
        every { ratingRepository.findByCounselorId(2L) } returns listOf(
            mockk { every { rating } returns 5.0 }  // 더 높은 평점
        )
