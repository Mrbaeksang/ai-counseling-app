...
class NaverLoginApiTest
    ...
    fun `신규 사용자가 Naver OAuth로 로그인하면 새 계정이 생성되고 토큰이 발급된다`() {
        ...
        // 사용자가 DB에 생성되었는지 확인
        val createdUser = userRepository.findByProviderIdAndAuthProvider(naverUserInfo.providerId, AuthProvider.NAVER)
    ...
    fun `유효하지 않은 Naver OAuth 토큰으로 로그인하면 401 에러가 반환된다`() {
        ...
        coEvery { naverTokenVerifier.verifyToken(any()) } returns
            Mono.error(RuntimeException("Invalid naver token"))
    ...
    fun `Naver API가 에러 응답을 반환하면 401 에러가 반환된다`() {
        // Given
        coEvery { naverTokenVerifier.verifyToken(any()) } returns
            Mono.error(RuntimeException("resultcode: 024, message: Authentication failed"))
    ...
    fun `기존 Naver 사용자의 프로필 이미지가 변경되어도 기존 정보를 유지한다`() {
        ...
        val updatedNaverUserInfo =
            OAuthUserInfo(
                ...
                name = "새닉네임",
                provider = "NAVER",
                picture = "https://new-profile.jpg" // 새 프로필
            )
    ...
    fun `만료된 Naver 토큰으로 로그인 시도하면 401 에러가 반환된다`() {
        // Given
        coEvery { naverTokenVerifier.verifyToken(any()) } returns
            Mono.error(RuntimeException("token expired"))
