...
class KakaoLoginApiTest
    ...
    fun `신규 사용자가 Kakao OAuth로 로그인하면 새 계정이 생성되고 토큰이 발급된다`() {
        ...
        // 사용자가 DB에 생성되었는지 확인
        val createdUser = userRepository.findByProviderIdAndAuthProvider(kakaoUserInfo.providerId, AuthProvider.KAKAO)
    ...
    fun `유효하지 않은 Kakao OAuth 토큰으로 로그인하면 401 에러가 반환된다`() {
        ...
        coEvery { kakaoTokenVerifier.verifyToken(any()) } returns
            Mono.error(RuntimeException("Invalid kakao token"))
    ...
    fun `Kakao 계정에 이메일이 없으면 providerId@kakao_local 형식으로 생성된다`() {
        ...
        val noEmailKakaoUserInfo =
            OAuthUserInfo(
                ...
                email = "", // Kakao는 이메일이 선택사항, 빈 문자열로 처리
                name = "이메일없는유저",
                provider = "KAKAO",
        ...
        val googleUser =
            userRepository.save(
                com.aicounseling.app.domain.user.entity.User(
                    ...
                    providerId = kakaoUserInfo.providerId, // 우연히 같은 ID (실제로는 거의 불가능)
    ...
    fun `Kakao 닉네임이 20자를 초과하면 20자로 잘라서 저장된다`() {
        ...
        val longNameKakaoUserInfo =
            OAuthUserInfo(
                ...
                name = "매우매우매우매우매우매우매우긴닉네임이예요정말로", // 23자
                provider = "KAKAO",
                picture = null
            )
